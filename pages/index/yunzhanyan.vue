<template>
	<view class="page" >
		...
		<swiper class="swiper" :vertical="true" @change="changePage" v-if="div001==0" :current="pageIndex" style="margin-left: -3.5%;  text-align: center">
...
<!-- 			<swiper-item>
				...
				<image class="imgYun" src="../../static/img/2.jpg" mode="scaleToFill"></image>
				<view class="huadong">
					<image src="../../static/img/jiantou.png" class="jiantou" mode=""></image>
					<image src="../../static/img/shou.png" class="shou bounceInUp" mode=""></image>
				</view>
			</swiper-item> -->
		 ...
		 :current="currentIndex">
			<swiper-item v-for="(item,index) in videoList">
				...
				<view class="videoBox1">
					<video class="video" :id="item.id" :ref="item.id" :src="item.src" controls="false" :loop="true" webkit-playsinline="true"
...
<script>
	...
	export default {
		...
		onReady() {
			...
			uni.request({
				url: "./static/data.json", //仅为示例，并非真实接口地址。
				dataType: 'json',
				method: 'GET',
				header: {
						...
						for (let i in this.videoList) {
							for (let j in shuju) {
								if (this.videoList[i].path == shuju[j].url) {
									let dianzhan = (localStorage.getItem("indexY" + i) == null) ? 0 : 1;
		...
		methods: {
			share() {
				...
				let shareUrl = this.GetUrlRelativePath() + "yunzhanyan?videoID=" + this._videoDataIndex;
					...
					uni.request({
						...
						success: (res) => {
							...
							if (res.data.code == 0) {
								...
								localStorage.setItem("indexY" + index, this.videoList[index].x);
								//this.updateDian();
...
.videoBox1 {
	...
	.divVideo{
		background-image: url(../../static/img/beijingdise.png);
